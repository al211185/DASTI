    ...
    <FieldArray name="renglones">
      {({ push, remove }) => (
        <div className="bg-white rounded-2xl shadow-lg p-6 mb-6 border">
          {/* Encabezado de sección + botón “Agregar renglón” (solo si no es Jefe, no es readOnly) */}
          {!isJefe && !readOnly && (
            <div className="flex justify-between items-center mb-4">
              ...
              <button
                ...
                className="bg-primary hover:bg-primary-dark text-white font-medium px-4 py-2 rounded-lg shadow transition"
              >
                Agregar renglón
              </button>
            </div>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-100">
                <tr>
                  <th className="w-16 px-2 py-2 text-left text-xs font-medium text-gray-500 uppercase whitespace-nowrap">
                    Cant
                  </th>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {renglones.map((r, idx) => (
                  <tr key={idx}>
                    ...
                    {/** Documentos (todos pueden ver, pero no subir si es readOnly) **/}
                    ...
                    <td className="p-2 border">
                      {isJefe || readOnly ? (
                        /* En modo Jefe o readOnly, todo es solo lectura */
                        <div className="space-y-4">
                          {/* Producción (solo mostrar) */}
                        ...
                        /* Vendedor (no Diseñador): solo lectura de todos los tiempos */
                        r.tiempos.map((t, iT) => (
                          <div key={iT} className="text-sm mb-1">
                            {t.maquina}: {t.horas}h
                          </div>
                        ...
                        /* Resto de roles (Diseñador, Admin, etc.): editable */
                        <div className="space-y-4">
                          {/* Producción editable */}
                          <section className="border border-green-300 bg-green-50 p-2 rounded">
                            <h4 className="font-semibold text-green-700 mb-2">Producción</h4>
                    ...
                    <td className="p-2 border text-center">
                      <button
                        ...
                        className={`px-2 py-1 rounded text-sm ${
                          readOnly ? 'bg-gray-100 text-gray-500 cursor-not-allowed' : 'bg-gray-200'
                        }`}
                        onClick={() => {
                          if (!readOnly) setModalComentariosIndex(idx);
                        }}
                        disabled={readOnly}
                      >
